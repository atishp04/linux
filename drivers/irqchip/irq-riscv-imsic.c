...
#ifdef CONFIG_SMP
static void imsic_ipi_send_mask(unsigned int parent_virq,
...
static void imsic_ipi_clear(unsigned int parent_virq)
{
	/* Nothing to do here. */
}

static struct ipi_mux_ops imsic_ipi_ops = {
	.ipi_mux_clear = imsic_ipi_clear,
	.ipi_mux_send = imsic_ipi_send_mask,
};

static void imsic_ipi_enable(struct imsic_priv *priv)
{
	__imsic_id_enable((void *)(unsigned long)priv->ipi_id);
}

static void imsic_ipi_disable(struct imsic_priv *priv)
{
	__imsic_id_disable((void *)(unsigned long)priv->ipi_id);
}
...
static int __init imsic_ipi_domain_init(struct imsic_priv *priv)
{
	...
	virq = ipi_mux_create(0, &imsic_ipi_ops);
	if (virq <= 0) {
...
static int __init imsic_ipi_domain_init(struct imsic_priv *priv)
{
	/* Clear the IPI interrupt identity because we are not using IPIs */
	priv->ipi_id = 0;
	return 0;
}
			     ...
			     struct device_node *parent)
{
	...
	if (of_property_read_u32(node, "riscv,ipi-id", &priv->ipi_id))
		priv->ipi_id = 0;
