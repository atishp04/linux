...
static struct smmc_dev_res *smmc_find_dres_for_gsi(struct smmc_data *data, int hwirq)
{
...
static void smmc_msi_irq_unmask(struct irq_data *d)
{
	...
	struct smmc_dev_res *devres = smmc_find_dres_for_gsi(data, d->hwirq);
...
static int smmc_parse_package_resource(struct platform_device *pdev, struct smmc_data *data)
{
	...
	if (!smmc_obj || smmc_obj->package.count != 2) {
	...
	for (i = 0; i < smmc_obj->package.count; i++) {
		...
		res->regs = devm_platform_ioremap_resource(pdev, i);
		if (IS_ERR(res->regs)) {
			result = PTR_ERR(res->regs);
			//TODO: Free up the list/memory for cfg
			goto end;
		}
